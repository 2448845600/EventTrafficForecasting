...
class STIDEF(nn.Module):
    def __init__(self, hist_len, pred_len, var_num, block_num, eod_size, ts_emb_dim, node_emb_dim, tod_emb_dim, dow_emb_dim, eod_emb_dim,
                 freq):
        super(STIDEF, self).__init__()
        ...
        self.eod_emb_dim = eod_emb_dim
        self.freq = freq

        self.tod_size = int((24 * 60) / self.freq)
        self.dow_size = 7
        self.eod_size = eod_size
        self.hidden_dim = self.ts_emb_dim + self.node_emb_dim + self.tod_emb_dim + self.dow_emb_dim + self.eod_emb_dim
        ...
        self.eod_emb = nn.Parameter(torch.empty(self.eod_size, self.eod_emb_dim))
    ...
    def _init_embeddings(self):
        ...
        nn.init.xavier_uniform_(self.eod_emb)
    ...
    def forward(self, var_x, marker_x):
        ...
        if self.eod_emb_dim:
            eod = (marker_x[:, -1, :, -1] * (self.eod_size - 1)).type(torch.LongTensor)
            eod_emb = self.eod_emb[eod].transpose(1, 2).repeat(1, 1, N)
